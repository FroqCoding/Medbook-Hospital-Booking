...
def ensure_schema():
    ...
    with app.app_context():
        ...
        # Use a single explicit transaction/connection for all DDL in SQLAlchemy 2.0
        # Engine.execute was removed; use engine.begin()/conn.execute(text(...)) instead.
        from sqlalchemy import text as _text
        def _exec(conn, sql):
            return conn.execute(_text(sql))
            ...
            if 'reason' not in cols:
                try:
                    with engine.begin() as conn:
                        _exec(conn, 'ALTER TABLE appointments ADD COLUMN reason VARCHAR')
        ...
        if 'doctors' in insp.get_table_names():
            ...
            try:
                with engine.begin() as conn:
                    _exec(conn, 'CREATE INDEX IF NOT EXISTS idx_doctors_approval_status ON doctors(approval_status)')
            except Exception:
                pass
            if backend == 'postgresql':
                ...
                try:
                    ...
                    if 'uq_doctor_slot' not in existing_uniques:
                        try:
                            with engine.begin() as conn:
                                _exec(conn, 'ALTER TABLE appointments ADD CONSTRAINT uq_doctor_slot UNIQUE (doctorid, appointment_date, appointment_time)')
                    ...
                    if not default_val:
                        try:
                            with engine.begin() as conn2:
                                _exec(conn2, 'ALTER TABLE appointments ALTER COLUMN appointmentid ADD GENERATED BY DEFAULT AS IDENTITY')
                            print('[ensure_schema] Added identity to appointmentid')
                        except Exception:
                            # Fallback sequence fix
                            try:
                                with engine.begin() as conn3:
                                    _exec(conn3, "CREATE SEQUENCE IF NOT EXISTS appointments_appointmentid_seq")
                                    _exec(conn3, "ALTER TABLE appointments ALTER COLUMN appointmentid SET DEFAULT nextval('appointments_appointmentid_seq')")
                                    _exec(conn3, "SELECT setval('appointments_appointmentid_seq', COALESCE((SELECT MAX(appointmentid) FROM appointments),0))")
        ...
        if 'doctor_reviews' in insp.get_table_names():
            ...
            if 'appointmentid' not in review_cols:
                try:
                    with engine.begin() as conn:
                        _exec(conn, 'ALTER TABLE doctor_reviews ADD COLUMN appointmentid INTEGER REFERENCES appointments(appointmentid)')
        ...
        if backend == 'postgresql':
            try:
                with engine.begin() as conn:
                    _exec(conn, 'CREATE INDEX IF NOT EXISTS idx_appointments_user_date ON appointments (userid, appointment_date)')
            except Exception:
                pass
            try:
                with engine.begin() as conn:
                    _exec(conn, 'CREATE INDEX IF NOT EXISTS idx_doctor_reviews_user_appt ON doctor_reviews (userid, appointmentid)')
            except Exception:
                pass

        # --- hospitals primary key identity/sequence fix (Postgres) ---
        if backend == 'postgresql' and 'hospitals' in insp.get_table_names():
            try:
                # Ensure hospitalid has identity/sequence and sequence is advanced to MAX(hospitalid)
                from sqlalchemy import text as _t
                with engine.begin() as conn:
                    seq_row = conn.execute(_t("SELECT pg_get_serial_sequence('hospitals','hospitalid')"))
                    seq_name = seq_row.scalar() if seq_row else None
                    if not seq_name:
                        # Try to add identity if no default present
                        col_def = conn.execute(_t("""
                            SELECT column_default FROM information_schema.columns
                            WHERE table_name='hospitals' AND column_name='hospitalid'
                        """)).scalar()
                        if not col_def:
                            try:
                                _exec(conn, 'ALTER TABLE hospitals ALTER COLUMN hospitalid ADD GENERATED BY DEFAULT AS IDENTITY')
                            except Exception:
                                pass
                            # re-fetch potential sequence name
                            seq_row2 = conn.execute(_t("SELECT pg_get_serial_sequence('hospitals','hospitalid')"))
                            seq_name = seq_row2.scalar() if seq_row2 else None
                    # If we have a sequence, set it to max(id)
                    if seq_name:
                        try:
                            _exec(conn, f"SELECT setval('{seq_name}', COALESCE((SELECT MAX(hospitalid) FROM hospitals), 0))")
                            print('[ensure_schema] synchronized hospitals sequence to MAX(hospitalid)')
                        except Exception as _e:
                            print('[ensure_schema] failed to synchronize hospitals sequence:', _e)
            except Exception as e:
                print('[ensure_schema] hospitals identity/sequence patch error:', e)

        # --- doctors primary key identity/sequence fix (Postgres) ---
        if backend == 'postgresql' and 'doctors' in insp.get_table_names():
            try:
                from sqlalchemy import text as _t
                with engine.begin() as conn:
                    seq_row = conn.execute(_t("SELECT pg_get_serial_sequence('doctors','doctorid')"))
                    seq_name = seq_row.scalar() if seq_row else None
                    if not seq_name:
                        col_def = conn.execute(_t("""
                            SELECT column_default FROM information_schema.columns
                            WHERE table_name='doctors' AND column_name='doctorid'
                        """)).scalar()
                        if not col_def:
                            try:
                                _exec(conn, 'ALTER TABLE doctors ALTER COLUMN doctorid ADD GENERATED BY DEFAULT AS IDENTITY')
                            except Exception:
                                pass
                            seq_row2 = conn.execute(_t("SELECT pg_get_serial_sequence('doctors','doctorid')"))
                            seq_name = seq_row2.scalar() if seq_row2 else None
                    if seq_name:
                        try:
                            _exec(conn, f"SELECT setval('{seq_name}', COALESCE((SELECT MAX(doctorid) FROM doctors), 0))")
                            print('[ensure_schema] synchronized doctors sequence to MAX(doctorid)')
                        except Exception as _e:
                            print('[ensure_schema] failed to synchronize doctors sequence:', _e)
            except Exception as e:
                print('[ensure_schema] doctors identity/sequence patch error:', e)

        # --- doctor_availability primary key identity/sequence fix (Postgres) ---
        if backend == 'postgresql' and 'doctor_availability' in insp.get_table_names():
            try:
                from sqlalchemy import text as _t
                with engine.begin() as conn:
                    seq_row = conn.execute(_t("SELECT pg_get_serial_sequence('doctor_availability','dayid')"))
                    seq_name = seq_row.scalar() if seq_row else None
                    if not seq_name:
                        col_def = conn.execute(_t("""
                            SELECT column_default FROM information_schema.columns
                            WHERE table_name='doctor_availability' AND column_name='dayid'
                        """
                        )).scalar()
                        if not col_def:
                            try:
                                _exec(conn, 'ALTER TABLE doctor_availability ALTER COLUMN dayid ADD GENERATED BY DEFAULT AS IDENTITY')
                            except Exception:
                                pass
                            seq_row2 = conn.execute(_t("SELECT pg_get_serial_sequence('doctor_availability','dayid')"))
                            seq_name = seq_row2.scalar() if seq_row2 else None
                    if seq_name:
                        try:
                            _exec(conn, f"SELECT setval('{seq_name}', COALESCE((SELECT MAX(dayid) FROM doctor_availability), 0))")
                            print('[ensure_schema] synchronized doctor_availability sequence to MAX(dayid)')
                        except Exception as _e:
                            print('[ensure_schema] failed to synchronize doctor_availability sequence:', _e)
            except Exception as e:
                print('[ensure_schema] doctor_availability identity/sequence patch error:', e)
...
def register_doctor():
    data = request.get_json() or {}
    # Allow either an existing hospitalid or a hospital name (legacy 'hospital' or new 'hospital_new')
    required = ['name','email','phone','speciality','password']
    missing = [k for k in required if not str(data.get(k,'' )).strip()]
    if missing:
        return json_error('Missing: '+', '.join(missing), 400)
    try:
        # Determine hospital: by id, or create/find by name
        hospital = None
        hosp_id_val = (data.get('hospitalid') or '').strip() if isinstance(data.get('hospitalid'), str) else data.get('hospitalid')
        if hosp_id_val:
            try:
                hospital = Hospital.query.get(int(hosp_id_val))
            except Exception:
                hospital = None
        if not hospital:
            hosp_new = (data.get('hospital_new') or '').strip()
            hosp_legacy = (data.get('hospital') or '').strip()
            hosp_name = hosp_new or hosp_legacy
            if not hosp_name:
                return json_error('Missing: hospital', 400)
            # Try find by case-insensitive name first
            hospital = Hospital.query.filter(Hospital.name.ilike(hosp_name)).first()
            if not hospital:
                hospital = Hospital(name=hosp_name)
                db.session.add(hospital)
                db.session.flush()  # get hospitalid
...
# ----------------------
# Hospitals list (for signup dropdown)
# ----------------------
@app.route('/hospitals', methods=['GET'])
def list_hospitals():
    rows = Hospital.query.order_by(Hospital.name.asc()).all()
    return jsonify([{'hospitalid': h.hospitalid, 'name': h.name} for h in rows])
